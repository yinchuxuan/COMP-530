...
public:
		...
		ValueList (vector<ExprTreePtr>& list) {
...
public:
	...
	vector<ExprTreePtr>& getSelectedValues() {
	...
	vector<ExprTreePtr>& getGrouping() {
...
public:
	...
	struct SFWQuery* getQuery() {
		return &myQuery;
	}

	struct CreateTable* getCreateTable() {
		return &myTableToCreate;
	}
